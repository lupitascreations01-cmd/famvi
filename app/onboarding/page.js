...
export default function Onboarding() {
  ...
  const toggleCategoria = (nombre) => {
    setCategorias(prev => prev.includes(nombre)
      ? prev.filter(c => c !== nombre)
      : [...prev, nombre])
  }

  const toggleDia = (dia) => {
    setDias(prev => prev.includes(dia)
      ? prev.filter(d => d !== dia)
      : [...prev, dia])
  }
  ...
  const irPaso2 = async () => {
    ...
    if (!whatsappHijo.trim()) { setError('Por favor ingresa tu WhatsApp'); return }
    setError('')
    // Guardar datos del hijo en Supabase
    setCargando(true)
    try {
  ...
  const irPaso3 = () => {
    ...
    if (!whatsappFamiliar.trim()) { setError('Por favor ingresa el WhatsApp de tu familiar'); return }
    setError('')
    setPaso(3)
  }
  ...
  // PASO 1 — Datos del hijo
  if (paso === 1) return (
    <main style={fondoBase}>
      {logo}
      {card(
        <>
          {pasoLabel(1, 3, 'Tus datos')}
          <h2 style={{fontSize:'1.5rem', fontWeight:'400', marginBottom:'0.4rem'}}>Cuéntanos sobre ti</h2>
          <p style={{color:'#6B7280', fontSize:'0.88rem', marginBottom:'1.5rem'}}>Primero tus datos para enviarte alertas.</p>
          <input placeholder="Tu nombre completo" value={nombreHijo} onChange={e => setNombreHijo(e.target.value)} style={estiloInput} />
          <input placeholder="Tu WhatsApp (+1 555 000 0000)" value={whatsappHijo} onChange={e => setWhatsappHijo(e.target.value)} style={estiloInput} />
  ...
  // PASO 2 — Datos del familiar
  if (paso === 2) return (
    <main style={fondoBase}>
      {logo}
      {card(
        <>
          {pasoLabel(2, 3, 'Tu familiar')}
          ...
          <input placeholder="WhatsApp de tu familiar (+52 555 000 0000)" value={whatsappFamiliar} onChange={e => setWhatsappFamiliar(e.target.value)} style={estiloInput} />
          <input placeholder="Ciudad donde vive (ej. Ciudad de México)" value={ciudad} onChange={e => setCiudad(e.target.value)} style={estiloInput} />
          {error && <div style={{background:'#FEE2E2', borderRadius:'10px', padding:'0.75rem', marginBottom:'1rem', fontSize:'0.82rem', color:'#E76F51'}}>{error}</div>}
