...
const PLANES = [
  {
    ...
    colorBtn: 'white',
    features: [
      '1 familiar monitoreado',
      'Check-ins diarios por WhatsApp',
      'Alertas inmediatas',
      'Dashboard web',
      'Historial 30 días'
    ]
  ...
  {
    nombre: 'Familiar',
    familiares: 'hasta 3 familiares',
    mensual: 17.99,
    anual: 179,
    anualMes: 14.92,
    urlMensual: 'https://famvi.lemonsqueezy.com/checkout/buy/118b895c-621b-49d9-be87-f30e0d8ab6c2',
    urlAnual: null,
    color: '#2D6A4F',
    borde: '#2D6A4F',
    badge: 'Más popular',
  ...
  {
    nombre: 'Premium',
    familiares: 'hasta 6 familiares',
    mensual: 24.99,
    anual: 249,
    anualMes: 20.75,
    urlMensual: 'https://famvi.lemonsqueezy.com/checkout/buy/a8effdfa-461e-4a53-a354-ae17d6e291d5',
    urlAnual: null,
    color: 'white',
    borde: '#E5E7EB',
    badge: null,
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        {PLANES.map(plan => (
          <div key={plan.nombre} style={{ background: plan.color, borderRadius: '20px', padding: '1.5rem', marginBottom: '1rem', border: `2px solid ${plan.borde}`, position: 'relative' }}>
            {plan.badge && (
              <span style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: '#74C69D', color: '#1A1A2E', padding: '0.25rem 1rem', borderRadius: '20px', fontSize: '0.75rem', fontWeight: '600' }}>
                {plan.badge}
              </span>
            )}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '1.8rem', fontWeight: '600', color: plan.nombre === 'Familiar' ? '#74C69D' : '#2D6A4F', lineHeight: 1 }}>
                  ${ciclo === 'mensual' ? plan.mensual : plan.anualMes}
                </div>
                <div style={{ fontSize: '0.75rem', color: plan.nombre === 'Familiar' ? 'rgba(255,255,255,0.6)' : '#6B7280' }}>
                  /mes{ciclo === 'anual' ? ' · cobrado anual' : ''}
                </div>
                {ciclo === 'anual' && (
                  <div style={{ fontSize: '0.72rem', color: '#74C69D', fontWeight: '500' }}>Total ${plan.anual}/año</div>
                )}
              </div>
            </div>
